...
async function addWidgetToGrid(w) {
  ...
  grid.addWidget(el);
  // Add hover hint + double-click to enter interactive focus mode
  const _wbody = el.querySelector('.widget-body');
  if (_wbody) {
    const _hint = document.createElement('div');
    _hint.className = 'widget-hover-hint';
    _hint.innerHTML = '<i class="fa fa-mouse-pointer" style="margin-right:5px"></i>Double-click to style';
    _wbody.appendChild(_hint);
    _wbody.addEventListener('dblclick', (e) => {
      if (currentMode !== 'edit') return;
      e.stopPropagation();
      wsbFocus(w.uid);
    });
  }
  // Double-click widget title to rename inline
  const _wtitle = el.querySelector('.widget-title');
  if (_wtitle) _attachInlineTitle(w.uid, _wtitle);
...
function renderPlotly(container, chartType, data, config, _plotCfgOverride) {
  ...
  // ── Visual style props (set by the in-widget style toolbar) ──────────────
  const barOpacity    = config.bar_opacity    !== undefined ? config.bar_opacity    : 1;
  const barGap        = config.bar_gap        !== undefined ? config.bar_gap        : 0.2;
  const lineWidth     = config.line_width     !== undefined ? config.line_width     : 2;
  const pointSize     = config.point_size     !== undefined ? config.point_size     : 8;
  const showGrid      = config.show_grid      !== false;
  const barOrientH    = config.bar_orient     === 'h';
  const fillArea      = config.fill_area      !== undefined ? config.fill_area      : (chartType === 'area');
  ...
  const gridColor = showGrid ? '#2a2f45' : 'transparent';
  layout.xaxis.gridcolor = gridColor;
  layout.yaxis.gridcolor = gridColor;
  layout.bargap = barGap;
  const plotConfig = _plotCfgOverride || { responsive: true, displayModeBar: false };
  let traces = [];
  ...
  if (seriesCol && yCols.length > 0) {
    ...
    seriesKeys.forEach((sk, i) => {
      ...
      traces.push(makeTrace(chartType, x, y, sk, colors[i % colors.length], isLine, isBar, isScatter, showLabels, showMarkers, lineShape, i, 'y', barOpacity, lineWidth, pointSize, barOrientH, fillArea));
    });
  } else {
    yCols.forEach((yCol, i) => {
      ...
      traces.push(makeTrace(chartType, x, y, yCol, colors[i % colors.length], isLine, isBar, isScatter, showLabels, showMarkers, lineShape, i, 'y', barOpacity, lineWidth, pointSize, barOrientH, fillArea));
    });
  }
  ...
  if (hasY2) {
    ...
    const t2 = makeTrace(chartType, x2, y2, y2Label, y2Color, isLine, isBar, isScatter, showLabels, showMarkers, lineShape, traces.length, 'y2', barOpacity, lineWidth, pointSize, barOrientH, fillArea);
...
function makeTrace(chartType, x, y, name, color, isLine, isBar, isScatter, showLabels, showMarkers, lineShape, idx, yaxis, barOpacity=1, lineWidth=2, pointSize=8, barOrientH=false, fillArea=false) {
  const trace = { x, y, name };
  if (yaxis) trace.yaxis = yaxis;  // 'y' (primary) or 'y2' (secondary)
  if (isBar) {
    trace.type = 'bar';
    trace.marker = { color, opacity: barOpacity };
    if (barOrientH) {
      trace.orientation = 'h';
      trace.x = y; trace.y = x; // swap axes for horizontal
    }
    if (showLabels) { trace.text = y.map(v => v !== null ? formatNum(v) : ''); trace.textposition = 'outside'; }
  } else if (isScatter) {
    trace.type = 'scatter';
    trace.mode = 'markers';
    trace.marker = { color, size: pointSize };
  } else if (chartType === 'area') {
    trace.type = 'scatter';
    trace.mode = showMarkers ? 'lines+markers' : 'lines';
    trace.fill = idx === 0 ? 'tozeroy' : 'tonexty';
    trace.line = { color, width: lineWidth, shape: lineShape };
...
// ═══════════════════════════════════════════════════════════════════════════
// IN-WIDGET STYLE TOOLBAR (Excel/PPT-style live editing)
// Double-click any chart widget body to enter focus mode.
// A floating toolbar appears with live style controls — all changes apply
// instantly via Plotly.restyle/relayout and auto-save to the server.
// ═══════════════════════════════════════════════════════════════════════════

let _wsbUid      = null;   // currently focused widget uid
let _wsbSaveTimer = null;  // debounce for server save
let _wsbData     = null;   // cached dataset for re-renders

// ── Focus a widget ─────────────────────────────────────────────────────────
async function wsbFocus(uid) {
  if (_wsbUid === uid) return;
  if (_wsbUid) wsbExit(false);   // leave previous without re-render

  _wsbUid = uid;
  const w = widgets.find(x => x.uid === uid);
  if (!w || !['bar','line','area','scatter','pie','donut'].includes(w.chart_type)) {
    _wsbUid = null; return;
  }

  const card = document.getElementById('wcard-' + uid);
  if (!card) return;
  card.classList.add('wfocused');
  document.getElementById('dashCanvas').classList.add('canvas-has-focus');

  // Re-render with Plotly toolbar enabled
  await _wsbRender(w);
  _wsbBuildBar(w, card);
}

// ── Exit focus mode ────────────────────────────────────────────────────────
function wsbExit(doRenderNormal = true) {
  if (!_wsbUid) return;
  const uid = _wsbUid;
  _wsbUid = null; _wsbData = null;
  clearTimeout(_wsbSaveTimer);

  const card = document.getElementById('wcard-' + uid);
  if (card) card.classList.remove('wfocused');
  document.getElementById('dashCanvas').classList.remove('canvas-has-focus');

  const bar = document.getElementById('wStyleBar');
  if (bar) bar.classList.remove('wsb-visible');

  const w = widgets.find(x => x.uid === uid);
  if (w) {
    _wsbSaveNow(w);
    if (doRenderNormal) renderWidget(w);  // restore non-interactive render
  }
}

// ── Render the chart with Plotly toolbar visible ───────────────────────────
async function _wsbRender(w) {
  const body = document.getElementById('wbody-' + w.uid);
  if (!body) return;

  if (!_wsbData) {
    // Fetch data (reuse same filter logic as renderWidget)
    const config = w.config || {};
    const useDash = config.use_dashboard_filters !== false;
    const dashF = useDash ? normalizeFilters(dashboardFilters.filter(f => f.dataset_id === w.dataset_id)) : [];
    const widF  = useDash ? normalizeFilters(config.filters || []) : normalizeFilters(config.custom_filters || []);
    const filters = [...dashF, ...widF];
    const url = `/api/dataset/${w.dataset_id}/data?limit=2000${filters.length ? '&filters=' + encodeURIComponent(JSON.stringify(filters)) : ''}`;
    try {
      const res = await fetch(url);
      ...
      _wsbData = json;
    } catch(e) { console.warn('wsb fetch error', e); return; }
  }
  ...
  const cfg = _enrichConfigWithDateMeta(w.config || {}, w.dataset_id);
  renderPlotly(div, w.chart_type, _wsbData, cfg, {
    responsive: true,
    displayModeBar: true,
    displaylogo: false,
...
// ── Build and position the floating toolbar ────────────────────────────────
function _wsbBuildBar(w, card) {
  const bar = document.getElementById('wStyleBar');
  if (!bar) return;
  const config = w.config || {};
  const ct = w.chart_type;

  document.getElementById('wsbTitle').textContent = (w.title || 'Widget').slice(0, 18);

  // Palette swatches
  const palDiv = document.getElementById('wsbPalette');
  palDiv.innerHTML = '';
  Object.keys(PALETTES).forEach(name => {
    const c = PALETTES[name];
    const sw = document.createElement('div');
    sw.className = 'wsb-swatch' + (name === (config.palette || 'default') ? ' wsb-swatch-on' : '');
    sw.style.background = `linear-gradient(135deg, ${c[0]} 50%, ${c[2]} 50%)`;
    sw.title = name;
    sw.onclick = () => {
      palDiv.querySelectorAll('.wsb-swatch').forEach(s => s.classList.remove('wsb-swatch-on'));
      sw.classList.add('wsb-swatch-on');
      wsbApply('palette', name);
    };
    palDiv.appendChild(sw);
  });

  // Show relevant group
  const isBar = ct === 'bar', isLine = ct === 'line' || ct === 'area';
  const isScatter = ct === 'scatter', isPie = ct === 'pie' || ct === 'donut';
  document.getElementById('wsbBarGroup').style.display    = isBar     ? 'flex' : 'none';
  document.getElementById('wsbLineGroup').style.display   = isLine    ? 'flex' : 'none';
  document.getElementById('wsbScatterGroup').style.display = isScatter ? 'flex' : 'none';
  document.getElementById('wsbPieGroup').style.display    = isPie     ? 'flex' : 'none';

  // Restore slider values
  if (isBar) {
    document.getElementById('wsbOpacity').value = config.bar_opacity !== undefined ? config.bar_opacity : 1;
    document.getElementById('wsbGap').value     = config.bar_gap     !== undefined ? config.bar_gap     : 0.2;
    document.getElementById('wsbHorizBtn').classList.toggle('wsb-on', config.bar_orient === 'h');
  }
  if (isLine) {
    document.getElementById('wsbLineW').value = config.line_width !== undefined ? config.line_width : 2;
    document.getElementById('wsbMarkersBtn').classList.toggle('wsb-on', config.show_markers !== false);
    document.getElementById('wsbSmoothBtn').classList.toggle('wsb-on', config.line_style === 'spline');
    document.getElementById('wsbFillBtn').classList.toggle('wsb-on', config.fill_area === true || ct === 'area');
  }
  if (isScatter) {
    document.getElementById('wsbPtSize').value = config.point_size !== undefined ? config.point_size : 8;
  }
  if (isPie) {
    document.getElementById('wsbPieHoleBtn').classList.toggle('wsb-on', ct === 'donut');
  }

  // Universal toggles
  document.getElementById('wsbGridBtn').classList.toggle('wsb-on', config.show_grid !== false);
  document.getElementById('wsbLegendBtn').classList.toggle('wsb-on', config.show_legend !== false);
  document.getElementById('wsbLabelsBtn').classList.toggle('wsb-on', !!config.show_labels);

  bar.classList.add('wsb-visible');
  _wsbPosition(card);
}

function _wsbPosition(card) {
  const bar = document.getElementById('wStyleBar');
  if (!bar || !card) return;
  const r = card.getBoundingClientRect();
  const bh = bar.offsetHeight || 46;
  const bw = bar.offsetWidth  || 480;
  let top  = r.bottom + 6;
  let left = r.left;
...
// ── Apply a style change live ──────────────────────────────────────────────
function wsbApply(prop, value) {
  if (!_wsbUid) return;
  const w = widgets.find(x => x.uid === _wsbUid);
  if (!w) return;
  if (!w.config) w.config = {};
  const c = w.config;
  const body = document.getElementById('wbody-' + _wsbUid);
  const gd   = body?.querySelector('.widget-plot');   // Plotly graph div

  switch (prop) {
    case 'palette': {
      c.palette = value;
      if (gd) {
        const cols = PALETTES[value];
        // restyle each trace individually
        const updates = (gd.data || []).map((_, i) => ({ 'marker.color': cols[i % cols.length] }));
        updates.forEach((upd, i) => Plotly.restyle(gd, upd, i));
        // also update fill colors for area
        (gd.data || []).forEach((tr, i) => {
          if (tr.fill) Plotly.restyle(gd, { fillcolor: hexToRgba(cols[i % cols.length], 0.15) }, i);
        });
      }
      break;
    }
    case 'opacity': {
      c.bar_opacity = value;
      if (gd) Plotly.restyle(gd, { 'marker.opacity': value });
      break;
    }
    case 'bargap': {
      c.bar_gap = value;
      if (gd) Plotly.relayout(gd, { bargap: value });
      break;
    }
    case 'orient': {
      c.bar_orient = c.bar_orient === 'h' ? 'v' : 'h';
      document.getElementById('wsbHorizBtn')?.classList.toggle('wsb-on', c.bar_orient === 'h');
      // orientation change needs full re-render
      if (_wsbData) {
        const gd2 = body?.querySelector('.widget-plot');
        if (gd2) {
          const cfg2 = _enrichConfigWithDateMeta(c, w.dataset_id);
          Plotly.purge(gd2);
          renderPlotly(gd2, w.chart_type, _wsbData, cfg2, { responsive:true, displayModeBar:true, displaylogo:false, modeBarButtonsToRemove:['sendDataToCloud','lasso2d','select2d','autoScale2d'], scrollZoom:true });
        }
      }
      break;
    }
    case 'lineWidth': {
      c.line_width = value;
      if (gd) Plotly.restyle(gd, { 'line.width': value });
      break;
    }
    case 'markers': {
      c.show_markers = !c.show_markers;
      document.getElementById('wsbMarkersBtn')?.classList.toggle('wsb-on', c.show_markers);
      if (gd) {
        const mode = c.show_markers ? 'lines+markers' : 'lines';
        Plotly.restyle(gd, { mode });
      }
      break;
    }
    case 'smooth': {
      c.line_style = c.line_style === 'spline' ? 'linear' : 'spline';
      document.getElementById('wsbSmoothBtn')?.classList.toggle('wsb-on', c.line_style === 'spline');
      if (gd) Plotly.restyle(gd, { 'line.shape': c.line_style });
      break;
    }
    case 'fill': {
      // toggle fill area (convert line ↔ area)
      if (w.chart_type === 'line') {
        w.chart_type = 'area'; c.fill_area = true;
      } else if (w.chart_type === 'area') {
        w.chart_type = 'line'; c.fill_area = false;
      }
      document.getElementById('wsbFillBtn')?.classList.toggle('wsb-on', w.chart_type === 'area');
      // full re-render needed
      _wsbData && _wsbReplotWith(w, body);
      break;
    }
    case 'ptSize': {
      c.point_size = value;
      if (gd) Plotly.restyle(gd, { 'marker.size': value });
      break;
    }
    case 'pieHole': {
      // toggle pie ↔ donut
      if (w.chart_type === 'pie') { w.chart_type = 'donut'; }
      else { w.chart_type = 'pie'; }
      document.getElementById('wsbPieHoleBtn')?.classList.toggle('wsb-on', w.chart_type === 'donut');
      _wsbData && _wsbReplotWith(w, body);
      break;
    }
    case 'grid': {
      c.show_grid = c.show_grid === false ? true : false;
      document.getElementById('wsbGridBtn')?.classList.toggle('wsb-on', c.show_grid !== false);
      if (gd) {
        const gc = c.show_grid !== false ? '#2a2f45' : 'transparent';
        Plotly.relayout(gd, { 'xaxis.gridcolor': gc, 'yaxis.gridcolor': gc });
      }
      break;
    }
    case 'legend': {
      c.show_legend = c.show_legend === false ? true : false;
      document.getElementById('wsbLegendBtn')?.classList.toggle('wsb-on', c.show_legend !== false);
      if (gd) Plotly.relayout(gd, { showlegend: c.show_legend !== false });
      break;
    }
    case 'labels': {
      c.show_labels = !c.show_labels;
      document.getElementById('wsbLabelsBtn')?.classList.toggle('wsb-on', !!c.show_labels);
      _wsbData && _wsbReplotWith(w, body);
      break;
    }
  }
  ...
  // Debounced save
  clearTimeout(_wsbSaveTimer);
  _wsbSaveTimer = setTimeout(() => _wsbSaveNow(w), 1400);
}

// Full re-plot inside the focused widget body (for changes that can't use restyle)
function _wsbReplotWith(w, body) {
  if (!body || !_wsbData) return;
  const gd = body.querySelector('.widget-plot');
  if (!gd) return;
  Plotly.purge(gd);
  const cfg = _enrichConfigWithDateMeta(w.config || {}, w.dataset_id);
  renderPlotly(gd, w.chart_type, _wsbData, cfg, {
...
async function _wsbSaveNow(w) {
  try {
    await fetch(`/api/widget/${w.uid}`, {
      method: 'PUT', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ title: w.title, chart_type: w.chart_type, dataset_id: w.dataset_id, config: w.config }),
    });
  } catch(e) { console.warn('wsb auto-save failed', e); }
}

function wsbDownload() {
  if (!_wsbUid) return;
  const body = document.getElementById('wbody-' + _wsbUid);
  const gd   = body?.querySelector('.widget-plot');
  if (!gd) return;
  const w = widgets.find(x => x.uid === _wsbUid);
  Plotly.downloadImage(gd, { format: 'png', filename: (w?.title || 'chart').replace(/\s+/g,'_'), scale: 2 });
}

// ── Inline title rename (double-click the widget title) ───────────────────
function _attachInlineTitle(uid, titleEl) {
  ...
  titleEl.addEventListener('dblclick', async (e) => {
    ...
    inp.type = 'text'; inp.value = w.title;
    inp.style.cssText = 'font:inherit;background:var(--bg4);border:1px solid var(--accent);border-radius:4px;padding:1px 6px;color:var(--text);width:180px;outline:none';
    titleEl.replaceWith(inp);
    inp.focus(); inp.select();
    const commit = async () => {
      const nt = inp.value.trim() || w.title;
      const sp = document.createElement('span');
      sp.className = 'widget-title'; sp.style.cursor = 'text'; sp.textContent = nt;
      inp.replaceWith(sp);
      _attachInlineTitle(uid, sp);
      if (nt !== w.title) {
        w.title = nt;
        await fetch(`/api/widget/${uid}`, {
          method: 'PUT', headers: {'Content-Type':'application/json'},
          body: JSON.stringify({ title: nt, chart_type: w.chart_type, dataset_id: w.dataset_id, config: w.config }),
        });
        const ft = document.getElementById('wsbTitle');
        if (_wsbUid === uid && ft) ft.textContent = nt.slice(0, 18);
      }
    };
    inp.addEventListener('blur', commit);
    inp.addEventListener('keydown', e2 => {
      if (e2.key === 'Enter') { e2.preventDefault(); inp.blur(); }
      if (e2.key === 'Escape') { inp.value = w.title; inp.blur(); }
    });
  });
}
...
document.addEventListener('keydown', e => {
  if (e.key === 'Escape' && _wsbUid) wsbExit();
});

document.addEventListener('click', e => {
  if (!_wsbUid) return;
  const bar  = document.getElementById('wStyleBar');
  const card = document.getElementById('wcard-' + _wsbUid);
  if (card?.contains(e.target) || bar?.contains(e.target)) return;
  if (e.target.closest('.side-panel,.panel-overlay')) return;
  wsbExit();
}, true);

window.addEventListener('resize', () => {
  if (!_wsbUid) return;
  _wsbPosition(document.getElementById('wcard-' + _wsbUid));
});